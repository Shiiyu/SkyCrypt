...
module.exports = {
  ...
  getItems: async (
    ...
    let armor =
      "inv_armor" in profile ? await getItems(profile.inv_armor.data, customTextures, packs, options.cacheOnly) : [];
    let inventory =
      "inv_contents" in profile
        ? await getItems(profile.inv_contents.data, customTextures, packs, options.cacheOnly)
        : [];
    let wardrobe_inventory =
      "wardrobe_contents" in profile
        ? await getItems(profile.wardrobe_contents.data, customTextures, packs, options.cacheOnly)
        : [];
    let enderchest =
      "ender_chest_contents" in profile
        ? await getItems(profile.ender_chest_contents.data, customTextures, packs, options.cacheOnly)
        : [];
    let talisman_bag =
      "talisman_bag" in profile
        ? await getItems(profile.talisman_bag.data, customTextures, packs, options.cacheOnly)
        : [];
    let fishing_bag =
      "fishing_bag" in profile
        ? await getItems(profile.fishing_bag.data, customTextures, packs, options.cacheOnly)
        : [];
    let quiver =
      "quiver" in profile ? await getItems(profile.quiver.data, customTextures, packs, options.cacheOnly) : [];
    let potion_bag =
      "potion_bag" in profile ? await getItems(profile.potion_bag.data, customTextures, packs, options.cacheOnly) : [];
    let candy_bag =
      "candy_inventory_contents" in profile
        ? await getItems(profile.candy_inventory_contents.data, customTextures, packs, options.cacheOnly)
        : [];
    let personal_vault =
      "personal_vault_contents" in profile
        ? await getItems(profile.personal_vault_contents.data, customTextures, packs, options.cacheOnly)
        : [];
    ...
    output.pickaxes = all_items.filter(
      (a) => a.type != null && (a.type.endsWith("pickaxe") || a.type.endsWith("drill"))
    );
    ...
    if (armor.filter((a) => Object.keys(a).length > 2).length == 4) {
      ...
      if (
        ...
        if (isPerfectSet) {
          const sameTier =
            armor.filter((a) => getId(a).split("_").pop() == getId(armor[0]).split("_").pop()).length == 4;
  ...
  getStats: async (
    ...
    for (let skill in levels) {
      ...
      const skillBonus = getBonusStat(
        levels[skill].level || levels[skill],
        `${skill}_skill`,
        levels[skill].levelCap,
        1
      );
    ...
    if (
      items.talismans.filter((a) => !a.isInactive && ["DAY_CRYSTAL", "NIGHT_CRYSTAL"].includes(getId(a))).length == 2
    ) {
  ...
  updateLeaderboardPositions: async (db, uuid, allProfiles) => {
    ...
    for (const slayer of getAllKeys(memberProfiles, "data", "slayer_bosses")) {
      for (const key of getAllKeys(memberProfiles, "data", "slayer_bosses", slayer)) {
        ...
        values[`${slayer}_slayer_boss_kills_tier_${tier}`] = getMax(
          memberProfiles,
          "data",
          "slayer_bosses",
          slayer,
